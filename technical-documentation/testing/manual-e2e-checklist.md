...
|------|-------------|----------|-----------|-------|
| 2026-09-09 | dev build, `issue-12-low-disk-resume` @ `2d41eef6` | macOS 26.3, Apple Silicon | **Pass — recording library and low-disk return flow** | Used isolated `OPENSCREEN_TEST_USER_DATA_DIR` data and `OPENSCREEN_LOW_DISK_STOP_BYTES=3000000000`; the real disk was not filled. A HUD-started capture stopped on the first 5 s poll and wrote `durationMs: 5007` plus `stopReason: low-disk`. From that editor project, Rec → Start recording stopped after 5.008 s, preserved `returnProjectId`, reopened the same project, and showed “録画をライブラリに保存しました”. The existing timeline stayed at one asset/clip and 5.1 s — the new take was not silently inserted. Media listed six takes newest first; the new 6 MB take displayed `0:05.0` and “容量不足前に保存”. Earlier in the same isolated run, normal recording return, explicit insertion, exact-path asset reuse, repeated insertion as separate clips, and duration probing before insertion also passed. No system audio, microphone, webcam, export, packaged-app behavior, Windows, or Linux was exercised. |
| 2026-09-09 | dev build, `issue-12-low-disk-resume` | macOS 26.3, Apple Silicon | **Pass — targeted low-disk continuation** | Ran in an isolated `OPENSCREEN_TEST_USER_DATA_DIR` and raised the development-only stop threshold above current free space, so no disk was deliberately filled. First capture auto-stopped after the 5 s storage poll and produced a playable 13 MB MP4 plus a 5.067 s project clip. From that editor project, Rec → Start recording produced a second capture; it auto-stopped, reopened the same project, displayed “Recording paused before storage ran out”, and appended a 5.010 s clip exactly at 5.067 s. Final project: 2 assets, 2 contiguous clips, 10.077 s total. No system audio, microphone, webcam, export, Windows, Linux, packaging, or real-volume exhaustion was exercised in this targeted run. |
