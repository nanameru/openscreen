...
| 2026-09-06 | local 1.10.0 / issue-9-preview-pacing | macOS / arm64 | Partial | Computer Useで保存済みプロジェクトの再読込、0:03.8→0:11.6の再生進行と映像更新、一時停止を確認。ユーザー操作を検出したためシーク以降は省略。実fpsの比較計測、録画異常停止の再現、エクスポート、他OSは未実施。アプリ署名検証成功。 |
|      |             |          |           |       |
